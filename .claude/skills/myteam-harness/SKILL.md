---
name: myteam-harness
description: "The merged execution loop for the myteam harness. Use when executing a PRD: iterate stories by priority, delegate each to its assigned agents (spawned via the Agent tool) in parallel, run quality gates, commit, and append learnings to progress.txt. Triggered by /myteam-do phase 2."
---
...
- Per-part context: each touched part's `CLAUDE.md` in its real repo directory (e.g.
  `Services/Basket/CLAUDE.md`) — Claude auto-loads these for the area you work in.
...
rules, the `config.yaml` context block, and the touched part's `CLAUDE.md` — then the **variable,
...
1. **Read state.** Open `prd.json` and the PRD `progress.txt`. Read the `## Codebase Patterns`
   ...
   learned. Then, for each part the story touches, read that part's repo-dir `CLAUDE.md`, and check
   recent history for the paths it will touch (e.g. `git log --oneline -n 15 -- <path>`,
   `git log -p -n 3 -- <file>`) to see how that code recently changed and why.
...
4. **Delegate to assigned agents (mandatory fresh-context subagent each, via the Agent tool).** Read
   the story's `agents[]`. For each `{ role, count }`, spawn `count` instances of that role using the
   **Agent tool** with `subagent_type: "<role>"` (the role names a file in `.claude/agents/`), capped
   by the story's `parallelism` (fallback `config.yaml > defaults.defaultParallelism`, now **1**).
   Default is a single agent; only fan out when the story is genuinely complex with independent
   sub-work. Run each on the story's `model` if set, else `config.yaml > defaults.model` (pass it as
   the Agent tool's `model` to keep grunt stories on the cheaper tier). Give each agent a **scoped,
   self-contained brief, ordered prefix-first** (stable: harness rules, `config.yaml` context, the
   relevant part's `CLAUDE.md`; then variable: story title, description, acceptance criteria, and the
   `git log` findings from step 1) — **not** the orchestrator's wider context or prior stories'
   transcripts.
   - Run independent agents concurrently by issuing multiple Agent tool calls in a single message;
     serialize where one depends on another's output.
...
10. **Consolidate.** Promote general, reusable learnings to the `## Codebase Patterns` section at
    the top of the PRD `progress.txt`; promote durable part-specific patterns into that part's repo-dir
    `CLAUDE.md` (which Claude Code auto-loads for that area).
