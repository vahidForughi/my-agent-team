/**
 * Checkout Module - Environment Configuration
 */
...
const getEnvVar = (key: string, defaultValue?: string): string => {
  ...
  if (envValue === undefined) {
    if (defaultValue === undefined) {
      console.warn(`[checkout/env] Variable ${key} is not set, using empty string`);
      return '';
    }
    return defaultValue;
  }
...
if (!env.apiBaseUrl) {
  throw new Error('[checkout] API Base URL is required');
}

if (env.useMockData) {
  console.warn('[checkout] 🚧 Running in MOCK DATA mode');
}
