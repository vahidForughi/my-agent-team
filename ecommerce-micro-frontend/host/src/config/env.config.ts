/**
 * Host Module - Environment Configuration
 */
...
const getEnvVar = (key: string, defaultValue?: string): string => {
  ...
  if (envValue === undefined) {
    if (defaultValue === undefined) {
      console.warn(`[host/env] Variable ${key} is not set, using empty string`);
      return '';
    }
    return defaultValue;
  }
...
if (!env.apiBaseUrl) {
  throw new Error('[host] API Base URL is required');
}

if (env.useMockData) {
  console.warn('[host] 🚧 Running in MOCK DATA mode');
}
