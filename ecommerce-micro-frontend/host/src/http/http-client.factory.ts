/**
 * Host Module - HTTP Client Factory
 */
...
export class HttpClientFactory {
  static create(options: HttpClientOptions = {}): AxiosInstance {
    ...
    instance.interceptors.request.use(
      ...
      (error) => {
        console.error('[host] ❌ Request interceptor error:', error);
  ...
  static createWithMock(
    ...
    if (env.useMockData && mockHandler) {
      instance.interceptors.request.use(
        async (config) => {
          console.warn('[host] 🚧 Using mock data for:', config.url);
