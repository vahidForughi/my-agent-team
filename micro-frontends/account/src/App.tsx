...
function InnerApp() {
  ...
  console.log('[InnerApp] Config from RouterContext:', config);
  console.log('[InnerApp] AppContext:', appContext);
  console.log('[InnerApp] User from useAuth:', user);
  ...
  const router = useMemo(() => {
    return createRouter({
      ...
      context: {
        ...
        config: undefined, // Will be set by RouterProvider context prop
      },
  ...
  return (
    <RouterProvider
      ...
      context={{
        queryClient,
        auth,
        config, // FIXED: Pass config to router context
      }}
    />
  );
}
...
function App(props: Props) {
  ...
  console.log('[App] AppContext:', appContext);
  ...
  console.log('[App] Auth config:', authConfig);
